...
const findElement = (value, index) =>
  ELEMENTS.find((element) => value.startsWith(element, index)) || "";

function parseFormulaBody(token) {
  let index = 0;
  let converted = false;
...
export function formatQuestionTextFields(question) {
  ...
  textKeys.forEach((key) => {
    if (next[key] != null && typeof next[key] !== "object") {
      if (key === "correctAnswer" && /^[A-Ea-e]$/.test(String(next[key]).trim())) {
        return;
      }
      next[key] = formatScientificText(next[key]);
    }
  });

  ["A", "B", "C", "D", "E"].forEach((id) => {
    [`option${id}`, `option${id}Text`, `option_${id}`].forEach((key) => {
      if (next[key] != null) next[key] = formatScientificText(next[key]);
    });
  });
  ...
  if (Array.isArray(next.options)) {
    next.options = next.options.map((option) =>
      option && typeof option === "object"
        ? { ...option, text: formatScientificText(option.text) }
        : option
    );
  }

  if (Array.isArray(next.subQuestions)) {
    next.subQuestions = next.subQuestions.map((subQuestion) =>
      formatQuestionTextFields(subQuestion)
    );
  }
